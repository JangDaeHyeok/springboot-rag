...
- **어댑터 교체는 `@ConditionalOnProperty`로 제어한다.**
  ...
  - `rag.query-preprocess.enabled=true|false`
...
└── LlmException         ← LLM/임베딩 호출 실패
     └── DouzoneEmbeddingException  ← Douzone 임베딩 API 오류 (EMBEDDING_FAILED, L0002)
...
| `spring.sql.init.mode` | `always` (`local` 프로파일) | `never` (기본값) |
...
./gradlew test --tests "*.AnalyticsControllerTest"           # 분석 컨트롤러 테스트 (산점도 포함)
```
