...
class RagControllerTest {
    ...
    void 정상_요청에서_200과_RAG_답변을_반환한다() throws Exception {
        ...
        when(ragAnswerService.answer(eq("세금 계산"), any(), anyBoolean())).thenReturn(mockResponse);
    ...
    void tenantId_domain_없어도_요청이_처리된다() throws Exception {
        ...
        when(ragAnswerService.answer(any(), any(), anyBoolean())).thenReturn(mockResponse);
    ...
    void 검색_결과_없어도_200으로_빈_citations_반환한다() throws Exception {
        when(ragAnswerService.answer(any(), any(), anyBoolean()))
                .thenReturn(new RagAnswerResponse("req-003",
                        "문서에서 확인되지 않습니다.", List.of()));

        RagController.AnswerRequest request = new RagController.AnswerRequest(
                "알 수 없는 질의", "default", null, null
        );
